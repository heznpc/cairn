...
export function landmarkIcon(category: LandmarkCategory, x: number, y: number, color: string): string {
  ...
  switch (category) {
    ...
    case "tram_stop":
      // Same car body as the bus, distinguished by the overhead wire and rails
      // — the two cues a reader actually uses to tell a tram from a bus.
      return `<g ${g}><rect x="${p(x - 6)}" y="${p(y - 6)}" width="12" height="12" rx="2"/><path d="M${p(x - 3)},${p(y - 1)} H${p(x + 3)}"/><path d="M${p(x)},${p(y - 6)} V${p(y - 10)}"/><path d="M${p(x - 8)},${p(y + 9)} H${p(x + 8)}"/></g>`;
    case "bus_stop":
    ...
    case "ferry":
      // Hull plus deckhouse: reads as "boat" at marker size without rigging.
      return `<g ${g}><path d="M${p(x - 9)},${p(y + 1)} H${p(x + 9)} L${p(x + 6)},${p(y + 8)} H${p(x - 6)} Z"/><path d="M${p(x - 4)},${p(y + 1)} V${p(y - 4)} H${p(x + 3)} V${p(y + 1)}"/><path d="M${p(x)},${p(y - 4)} V${p(y - 8)}"/></g>`;
    case "cafe":
    ...
    case "supermarket":
      // Trolley rather than the convenience-store bag, so the two shop classes
      // stay distinguishable side by side.
      return `<g ${g}><path d="M${p(x - 9)},${p(y - 6)} H${p(x - 6)} L${p(x - 3)},${p(y + 3)} H${p(x + 7)}"/><path d="M${p(x - 5)},${p(y - 2)} H${p(x + 9)} L${p(x + 7)},${p(y + 3)}"/><circle cx="${p(x - 2)}" cy="${p(y + 7)}" r="1.6"/><circle cx="${p(x + 6)}" cy="${p(y + 7)}" r="1.6"/></g>`;
    case "restaurant":
    ...
    case "pharmacy":
      // Ringed cross: the European pharmacy sign, and distinct from the
      // hospital's bare cross at a glance.
      return `<g ${g}><circle cx="${p(x)}" cy="${p(y)}" r="8"/><path d="M${p(x)},${p(y - 4.5)} V${p(y + 4.5)}"/><path d="M${p(x - 4.5)},${p(y)} H${p(x + 4.5)}"/></g>`;
    case "school":
...
export function markerStyle(
  ...
  switch (category) {
    case "station":
    // Tram and ferry are transit arrivals too, so they share the transit ink
    // and the emphasis that marks "this is where you get off".
    case "tram_stop":
    case "ferry":
